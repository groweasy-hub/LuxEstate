...
function from()       { return `"LuxEstate" <${process.env.SMTP_USER}>`; }
function adminEmail() { return process.env.ADMIN_EMAIL || process.env.SMTP_USER; }
              ...
              <tr>
                <td>
                  <div style="margin-bottom:6px;">
                    <span style="font-family:'Georgia',serif;font-size:30px;font-weight:300;letter-spacing:0.08em;color:#C9A84C;">Lux</span><span style="font-family:'Georgia',serif;font-size:30px;font-weight:700;letter-spacing:0.08em;color:#E8C96A;">Estate</span>
                  </div>
                  <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:10px;letter-spacing:0.25em;text-transform:uppercase;color:rgba(201,168,76,0.55);">Premium Real Estate &nbsp;·&nbsp; Since 2014</p>
                </td>
        ...
        <tr>
          <td style="background:#080808;padding:28px 48px;border:1px solid rgba(201,168,76,0.1);border-top:none;border-radius:0 0 4px 4px;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td>
                  <p style="margin:0 0 4px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:11px;color:rgba(201,168,76,0.5);letter-spacing:0.15em;text-transform:uppercase;">LuxEstate</p>
                  <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:11px;color:rgba(245,240,232,0.2);line-height:1.7;">&copy; 2026 LuxEstate. All rights reserved.<br/>Powered by Kommu Tech &amp; Marketing Pvt. Ltd</p>
                </td>
...
// ── Resolve image to absolute public URL ─────────────────────────────────────
function resolveImageUrl(url) {
  if (!url) return '';
  // Already absolute (Cloudinary, https, http)
  if (url.startsWith('http://') || url.startsWith('https://')) return url;
  // Local path — can't be loaded by email clients, skip it
  return '';
}
...
function buildUserEmail(lead, imageUrl, offerDiscount) {
  const hasOffer   = !!lead.offerTitle;
  const hasProject = !!lead.projectInterested;

  const subject = hasOffer
    ? `Your Exclusive Offer Enquiry — ${lead.offerTitle}`
    : hasProject
      ? `Your Property Enquiry — ${lead.projectInterested}`
      : 'Thank You for Contacting LuxEstate';

  const headline = hasOffer ? `Your Deal is Being Processed` : `We've Received Your Enquiry`;

  const intro = hasOffer
    ? `Thank you for your interest in the <strong style="color:#C9A84C;">${lead.offerTitle}</strong> offer. Our dedicated property expert will reach out within <strong style="color:#E8C96A;">2 hours</strong> to personally assist you in claiming this exclusive deal.`
    : hasProject
      ...
      : `Thank you for reaching out to LuxEstate. Our team will get back to you within <strong style="color:#E8C96A;">2 hours</strong>.`;
  ...
  const body = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:6px;margin-bottom:36px;">
      ...
      ${[['01','Our expert reviews your enquiry'],['02','You receive a call within 2 hours'],['03','Site visit scheduled at your convenience']].map(([n,t]) => `
      <tr><td style="padding:14px 20px;border-bottom:1px solid rgba(255,255,255,0.04);">
        <table cellpadding="0" cellspacing="0" border="0"><tr>
          <td style="padding-right:14px;vertical-align:top;"><span style="font-family:'Georgia',serif;font-size:18px;color:rgba(201,168,76,0.4);font-weight:300;">${n}</span></td>
          <td style="vertical-align:middle;"><p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:13px;color:rgba(245,240,232,0.6);">${t}</p></td>
        </tr></table>
      </td></tr>`).join('')}
    </table>
    ...
    <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:13px;color:rgba(245,240,232,0.35);line-height:1.7;">
      ...
      Call us at <a href="tel:+919999999999" style="color:#C9A84C;text-decoration:none;">+91 99999 99999</a> &nbsp;|&nbsp;
      <a href="mailto:hello@luxestate.in" style="color:#C9A84C;text-decoration:none;">hello@luxestate.in</a>
    </p>`;
...
function buildAdminEmail(lead, imageUrl, offerDiscount) {
...
// ── Main export ───────────────────────────────────────────────────────────────
async function sendLeadEmails(lead, { projectImage = '', offerImage = '', offerDiscount = '' } = {}) {
  ...
  // Prefer offer image, fall back to project image
  ...
  if (lead.email) {
    ...
    sends.push(
      transporter.sendMail({ from: from(), to: lead.email, subject, html })
        .then(() => console.log(`User email sent → ${lead.email}`))
        .catch((err) => console.error('User email error:', err.message))
    );
  }

  const { subject, html } = buildAdminEmail(lead, imageUrl, offerDiscount);
  sends.push(
    transporter.sendMail({ from: from(), to: adminEmail(), subject, html })
