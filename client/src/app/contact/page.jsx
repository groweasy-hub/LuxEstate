...
function FloatingField({ name, label, type = 'text', value, onChange, error, as = 'input', rows }) {
    ...
    <div className="relative" style={{ marginBottom: 'var(--space-6)' }}>
      <Tag
        ...
        style={{
          ...
          borderColor: focused ? 'var(--border-focus)' : error ? 'var(--color-accent-danger)' : undefined,
      ...
      <label
        style={{
          ...
          left: 'var(--space-4)',
          top: lifted ? 6 : '50%',
          transform: as === 'textarea' ? (lifted ? 'none' : 'translateY(calc(var(--space-4)))') : (lifted ? 'none' : 'translateY(-50%)'),
    ...
    return (
      ...
      >
        <svg width="72" height="72" viewBox="0 0 72 72" fill="none">
          <circle cx="36" cy="36" r="34" stroke="var(--color-gold)" strokeWidth="1.5" opacity="0.3" />
          <circle cx="36" cy="36" r="26" stroke="var(--color-gold)" strokeWidth="1.5" />
          <path
        ...
        <h3>Message Sent!</h3>
        <p className="lead" style={{ maxWidth: 320 }}>Our team will reach out within 24 hours.</p>
      </motion.div>
    );
  }
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 var(--space-4)' }}>
        <div style={{ gridColumn: '1' }}>
          <FloatingField name="name" label="Full Name" value={form.name} onChange={handleChange} error={errors.name} />
        </div>
        <div style={{ gridColumn: '2' }}>
          <FloatingField name="phone" label="Phone Number" type="tel" value={form.phone} onChange={handleChange} error={errors.phone} />
        </div>
      </div>
      <FloatingField name="email" label="Email Address" type="email" value={form.email} onChange={handleChange} error={errors.email} />
      <FloatingField name="message" label="Your Message" as="textarea" rows={5} value={form.message} onChange={handleChange} error={errors.message} />
...
const contactItems = [
  ...
  { icon: Mail, label: 'Email Us', value: 'hello@luxestate.in', href: 'mailto:hello@luxestate.in' },
  { icon: MapPin, label: 'Visit Us', value: 'Bandra West, Mumbai 400050', href: '#map' },
    ...
    <>
      ...
      >
        ...
        <div className="container relative text-center" style={{ zIndex: 1 }}>
          ...
          >
            Have a question or ready to find your dream home? We're here to help.
          </motion.p>
        </div>
      </section>
      ...
      <section className="section" style={{ background: 'var(--surface-page)' }}>
        <div
          className="container"
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-16)', alignItems: 'start' }}
        >
          {/* Info */}
          <div>
            <motion.div
            ...
            >
              ...
              <p className="lead mb-10">Reach out through any channel — we respond within hours.</p>
            </motion.div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              {contactItems.map(({ icon: Icon, label, value, href }, i) => (
                ...
                >
                  ...
                  <div>
                    <p className="overline" style={{ marginBottom: 2 }}>{label}</p>
                    {href ? (
                      <a href={href} className="hover-gold" style={{ color: 'var(--text-primary)', fontSize: 'var(--text-base)' }}>
                        {value}
                      </a>
                    ) : (
                      <span style={{ color: 'var(--text-primary)', fontSize: 'var(--text-base)' }}>{value}</span>
                    )}
                  </div>
                </motion.div>
              ))}
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.03, boxShadow: '0 0 24px rgba(37,211,102,0.35)' }}
              whileTap={{ scale: 0.97 }}
              animate={{ y: [0, -4, 0] }}
      ...
      {/* ── MAP ── */}
      <section id="map" className="section-sm" style={{ background: 'var(--surface-base)', borderTop: '1px solid var(--border-subtle)' }}>
        <div className="container">
          <motion.div
            ref={mapRef}
            data-reveal
            className="overflow-hidden"
            style={{ borderRadius: 'var(--radius-xl)', border: '1px solid var(--border-subtle)', height: 420 }}
